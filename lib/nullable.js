...
/** @type {<P, R>(nullable: Nullable<P>, func: F.Arrow<P, R>) => Nullable<R>} */
const map = (nullable, func) => isSome(nullable) ? some(func(nullable)) : nullable

/** @type {<P, R>(func: F.Arrow<P, R>) => (nullable: Nullable<P>) => Nullable<R>} */
const mapC = func => nullable => map(nullable, func)

/** @type {<P, R>(nullable: Nullable<P>, func: F.Arrow<P, Nullable<R>>) => Nullable<R>} */
const chain = (nullable, func) => isSome(nullable) ? func(nullable) : nullable

/** @type {<P, R>(func: F.Arrow<P, Nullable<R>>) => (nullable: Nullable<P>) => Nullable<R>} */
...
/** @type {<T1, T2>(nullable: Nullable<T1>, value: F.Lazy<T2>) => T1 | T2} */
...
/** @type {<T1>(value: F.Lazy<T1>) => <T2>(nullable: Nullable<T2>) => T1 | T2} */
const getOrElseLC = value => nullable => getOrElseL(nullable, value)

/** @type {<P, R, N>(nullable: Nullable<P>, onSome: F.Arrow<P, R>, onNull: N) => R | N} */
const fold = (nullable, onSome, onNull) => isSome(nullable) ? onSome(nullable) : onNull

/** @type {<P, R, N>(nullable: Nullable<P>, onSome: F.Arrow<P, R>, onNull: F.Lazy<N>) => R | N} */
const foldL = (nullable, onSome, onNull) => isSome(nullable) ? onSome(nullable) : onNull()

/** @type {<P, R, N>(onSome: F.Arrow<P, R>, onNull: N) => (nullable: Nullable<P>) => R | N} */
const foldC = (onSome, onNull) => nullable => fold(nullable, onSome, onNull)

/** @type {<P, R, N>(onSome: F.Arrow<P, R>, onNull: F.Lazy<N>) => (nullable: Nullable<P>) => R | N} */
const foldLC = (onSome, onNull) => nullable => foldL(nullable, onSome, onNull)

// SECTION Exports

module.exports = { some, isNull, isSome, map, mapC, chain, chainC, getOrElse, getOrElseL, getOrElseC, getOrElseLC, fold, foldL, foldC, foldLC }
