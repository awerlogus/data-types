...
describe('clone function', () => {
  ...
  it('should return new map map array object', () => {
    ...
    expect(MMA.clone(mapArray) === mapArray).toBe(false)
    expect(MMA.clone(mapArray).get(1) === mapArray.get(1)).toBe(false)
    expect(MMA.clone(mapArray).get(1)?.get(1) === mapArray.get(1)?.get(1)).toBe(false)
  })
})
