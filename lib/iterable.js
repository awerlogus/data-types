...
/** @deprecated @type {<T1, T2>(iter1: Iterable<T1>, iter2: Iterable<T2>) => Iterable<T1 | T2>} */
function* merge (iter1, iter2) { yield* iter1; yield* iter2 }

/** @deprecated @type {<T1>(iter1: Iterable<T1>) => <T2>(iter2: Iterable<T2>) => Iterable<T1 | T2>} */
const mergeC = iter1 => iter2 => merge(iter1, iter2)

/** @deprecated @type {<T2>(iter2: Iterable<T2>) => <T1>(iter1: Iterable<T1>) => Iterable<T1 | T2>} */
...
module.exports = { forEach, forEachC, forEachCR, map, mapC, mapCR, filter, filterC, filterCR, merge, mergeC, mergeCR, concat: merge, concatC: mergeC, concatCR: mergeCR, reduce, reduceC, reduceCR, fold, foldC, foldCR, countMatches, countMatchesC, countMatchesCR }
