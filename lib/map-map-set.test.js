...
describe('clone function', () => {
  ...
  it('should return new map set object', () => {
    ...
    expect(MMS.clone(mapSet)).not.toBe(mapSet)
    expect(MMS.clone(mapSet).get(1)).not.toBe(mapSet.get(1))
    expect(MMS.clone(mapSet).get(1)?.get(2)).not.toBe(mapSet.get(1)?.get(2))
  })
})
